...
// retrieve user information
    $id = $_SESSION["id"];
    $query = "SELECT first_name FROM tbl_dentist WHERE user_id='$id'";
...
// retrieve appointments for today
    $today = date('Y-m-d');
    $query_today = "SELECT * FROM tbl_patient WHERE DATE(ap_date) = '$today'";
    $result_today = mysqli_query($con, $query_today) or die("Error in query: " . mysqli_error($con));

// retrieve appointments for this week
    $week_start = date('Y-m-d', strtotime('last Monday'));
    $week_end = date('Y-m-d', strtotime('next Sunday'));
    $query_week = "SELECT * FROM tbl_patient WHERE DATE(ap_date) BETWEEN '$week_start' AND '$week_end'";
    $result_week = mysqli_query($con, $query_week) or die("Error in query: " . mysqli_error($con));

// retrieve appointments for this month
    $month_start = date('Y-m-01');
    $month_end = date('Y-m-t');
    $query_month = "SELECT * FROM tbl_patient WHERE DATE(ap_date) BETWEEN '$month_start' AND '$month_end'";
    $result_month = mysqli_query($con, $query_month) or die("Error in query: " . mysqli_error($con));
?>
    ...
    <div class="modal-dialog">
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-menu">
                <ul>
                    <li>
                        <a href="/DACclinic/dentist/dentist_dashboard.php" class="active"><span class="las la-home"></span>
                        <span>Dashboard</span></a>
                    </li>
                    <li>
                        <a href="/DACclinic/dentist/view_appointments.php"  ><span class="las la-user-circle"></span>
        ...
        <div class="main-content">
            <header>
                    ...
                    <div class="user-wrapper">
                        <div>
                        <h2>Welcome Dentist <?php echo"$firstName";?></h2>
                        </div>
                    </div>
            </header>
        <main>
            <hr>
                <div class="row d-flex justify-content-center">
                    <div class="col-3 appointments text-center current pt-2"><h5>Appointments Today: <?php echo mysqli_num_rows($result_today); ?></h5></div>
                    <div class="col-3 appointments text-center pt-2 "><h5>Appointments This week: <?php echo mysqli_num_rows($result_week); ?></h5></div>
                    <div class="col-3 appointments text-center pt-2 "><h5>Appointments This Month : <?php echo mysqli_num_rows($result_month); ?></h5></div>
                </div>        
            <hr>    
            <?php
        ...
        if(mysqli_num_rows($result_today) == 0) {
            ...
            while($row=mysqli_fetch_array($result_today)) {
                ...
                <tr> 
                    ...
                    <td>$comment</td>
                    <td>
                    <a type='button' class='btn btn-primary' data-bs-toggle='modal' data-bs-target='#staticBackdrop'>Done</a>
                </td>
                </tr>";
            } 
        }
